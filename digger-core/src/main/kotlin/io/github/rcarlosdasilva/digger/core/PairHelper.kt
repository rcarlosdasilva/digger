...
class PairHelper private constructor() {
  ...
  companion object {
     ...
     * ```
     * Pair<String, Integer> p1 = new Pair<>("first", 1);
     * Pair<String, Integer> p2 = new Pair<>("first", 2);
     * Pair<String, Integer> p3 = new Pair<>("first", 3);
     * Pair<String, Integer> p4 = new Pair<>("first", 4);
     * Pair<String, Integer> p5 = new Pair<>("first", 5);
     *
     * List<Pair<String, Integer>> pairs = Lists.newArrayList(p1, p2, p3, null, p4, p5);
     * // return [1, 2, 3, 4, 5]
     * List<Integer> list = PairHelper.toList(pairs);
     * ```
     * @param pairs Collection<Pair<F, S>> Pair集合
     * @return List<V> 只包含[Pair.second]的List
     */
    @JvmStatic
    fun <F, S> toList(pairs: Collection<Pair<F, S>>) =
        pairs.filterNotNull().map { it.second }
     ...
     * ```
     * @param pairs Collection<Pair<F, S>> Pair集合
     * @return Map<F, S> 转换后的Map
     */
    @JvmStatic
    fun <F, S> toMap(pairs: Collection<Pair<F, S>>) =
        pairs.filterNotNull().associate { kotlin.Pair(it.first, it.second) }
    ...
    fun assembly(pairs: Collection<Pair<*, *>>, associate: String, join: String) =
        pairs.filterNotNull().map { "${it.first}$associate${it.second}" }.joinToString(join)
